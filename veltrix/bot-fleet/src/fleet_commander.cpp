...
FleetCommander::FleetCommander(uint16_t    listen_port,
    ...
    , redpanda_brokers_(std::move(redpanda_brokers))
    , ioc_(1)
{}
...
void FleetCommander::launch_benchmark(const BenchmarkConfig& cfg) {
    ...
    // ── One shared Redpanda producer (thread-safe) ────────────────────────────
    auto producer = std::make_shared<TelemetryProducer>(
        redpanda_brokers_, cfg.telemetry_topic
    );
...
BenchmarkConfig FleetCommander::parse_benchmark_request(const std::string& body) {
    ...
    cfg.redpanda_brokers = redpanda_brokers_;

    if (cfg.num_bots <= 0)      cfg.num_bots = 1000;
    if (cfg.duration_secs <= 0) cfg.duration_secs = 60;
    if (cfg.protocol.empty())   cfg.protocol = "rest";
